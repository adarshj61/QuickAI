...
const BlogTitles = () => {
    const blogCategories = ['General', 'Technology', 'Health', 'Finance', 'Lifestyle'];
    ...
    const onSubmitHandler = async (e) => {
      ...
      try{
        setLoading(true);
        const prompt = `Generate blog titles for the category ${selectedCategory} with the keyword ${input}.`;
    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700  ">
      ...
      <form onSubmit={onSubmitHandler} className="w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200">
        ...
        <p className="mt-4 text-sm font-medium">Category</p>
          ...
          <br />
          <button disabled={loading} className="w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#C341F6] to-[#8E37EB] text-white text-sm rounded-lg cursor-pointer ">
